...
MatLib* getCandleGlass1() {
    ...
    ConstantTexture* t2 = new ConstantTexture(RGBColor(147, 255, 216) / 255.f);
...
MatLib* getCandleGlass2() {
    ...
    MatLib* glass = new MatLib;
    ConstantTexture* t2 = new ConstantTexture(RGBColor(255.f, 248.f, 154.f) / 255.f);
    CombineMaterialTex* m = new CombineMaterialTex();
    m->add(new GlassMaterial(1.45f), t2, .7f);
    m->add(new GlassMaterial(1.45f), nullptr, 0.3f);
...
MatLib* getCandleGlass3() {
    ...
    ConstantTexture* t2 = new ConstantTexture(RGBColor(255.f, 173.f, 240.f) / 255.f);
...
MatLib* getPlantJar() {
    ...
    Texture* t1 = new ImageTexture("models/candle2.png");
    ConstantTexture* t2 = new ConstantTexture(RGBColor(154.f, 230.f, 110.f) / 255.f);
    CombineMaterialTex* m = new CombineMaterialTex();
...
MatLib* getDust() {
    ...
    MatLib* dust = new MatLib;
    Texture* t1 = new ImageTexture("models/disco.png");
    ConstantTexture* t2 = new ConstantTexture(RGBColor::rep(1.0f));

    dust->insert(std::pair<std::string, Material*>("Material", new FlatMaterial(t1)));

    return dust;
}

float dust(int x, int y, int z)
{
    int n = x + y * 57 + z * 997;
    n = (n << 13) ^ n;
...
void create_dust(Group* g)
{
    CoordMapper* mapper = new QuadMapper(Vector::rep(-1.f), Vector::rep(4.0f));
    Texture* dustTex = new ImageTexture("models/disco.png");
    ConstantTexture* t2 = new ConstantTexture(RGBColor::rep(1.0f));
    
    for (int i = 0; i < 100; i++)
    {
        float sphere_radius = random(.01, .1); Point center = Point(random(-100, 100), random(-100, 100), random(-100, 200));
        float n;
        for (int j = 0; j <= 5; j++)
        {
            Point c = Point(center.x * j, center.y * j, center.z * j);
            g->add(new Sphere(c, sphere_radius, mapper, new GlassMaterial(1.45f)));
        }
...
void RC() {
    ...
    //{
    //    Instance* itree = new Instance(dust);
    //    itree->translate(Vector(random(-100, 100), random(-5, 5), random(-100, 100)));
    //    scene->add(itree);
    //}
    ...
    //MatLib* dust_ = getDust();
    //loadOBJ(scene, "models/", "dust.obj", nullptr, dust_);
    ...
    loadOBJ(scene, "models/", "ghost.obj", nullptr, ghost, true);
    ...
    img.writePNG("candle_glass_bm.png");
}