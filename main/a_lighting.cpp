...
namespace {
    ...
    void renderCornellboxB(float scale, const char* filename) {
        ...
        RayTracingIntegrator integrator(&world);
        std::cout << "Light sources: " << world.light.size() << std::endl;
        Renderer engine(&cam, &integrator);
        engine.render(img);
        img.writePNG(filename);
    }
    ...
    void renderCornellboxC(float scale, const char* filename) {
        ...
        //projective light
        //world.light.push_back(new ProjLight(Point(288 * scale, 529.99f * scale, 279.5f * scale), RGBColor::rep(80000.0f * scale * scale)));
