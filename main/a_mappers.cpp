...
void tryenvmapper(const char* filename, CoordMapper* envmapper) {
    static const float scale = 0.001f;
    Image img(400, 400);
    World world;
    SimpleGroup scene;
    ...
    ImageTexture* sphereTex = new ImageTexture("models/dark_forest.jpg");
    ImageTexture* envtex = new ImageTexture("models/f1.png");
    
    LambertianMaterial whitesphere(blacktex, sphereTex);
    ...
    // env solid
    scene.add(new Environment(envmapper, &env));
    //sphere
    //scene.add(new Sphere(Point(400.f, 450.f, 300.f) * scale, 150.f * scale, envmapper, &env));
    //scene.add(new Sphere(Point(200.f, 100.f, 300.f) * scale, 150.f * scale, envmapper, &whitesphere));
