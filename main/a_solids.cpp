...
void a_solids() {
    ...
    SimpleGroup* quadric_scene = new SimpleGroup();

    //sphere
    quadric_scene->add(new Quadric(Point(1, -1, 1), 1.0f, 1.0f, 1.0f, 1.0f, nullptr, nullptr));
    // ellipsoid
    quadric_scene->add(new Quadric(Point(-1, -1, 0), 2.5f, 1.5f, 5.0f, 1.0f, nullptr, nullptr));

    World world;
    world.scene = scene;

    World quadric_world;
    quadric_world.scene = quadric_scene;
    ...
    RayCastingDistIntegrator quadric_integrator(&quadric_world, RGBColor(1.0f, 0.2f, 0.0f), 4.0f, RGBColor(0.2f, 1.0f, 0.0f), 12.0f);
    Renderer quadric_engine(&cam, &quadric_integrator);
    quadric_engine.render(img);
    img.writePNG("quadric_a2-2.png");
}